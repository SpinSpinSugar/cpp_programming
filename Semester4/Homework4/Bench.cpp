...
int main(int argc, char* argv[]) {
    ...
    constexpr int NTASKS_KEEPER = 4096;
    ...
    {
        boost::lockfree::stack<int> lockfreeStack {NTASKS_KEEPER};
        //threadPerf::LockfreeStack<int> lockfreeStack{};
        std::vector<std::jthread> producers;
        std::vector<std::jthread> consumers;
        for (int i = 0; i < numberOfProducers; ++i)
            producers.emplace_back(produceLockfree<boost::lockfree::stack<int>>, std::ref(lockfreeStack));
        for (int i = 0; i < numberOfProducers; ++i)
            consumers.emplace_back(consumeLockfree<boost::lockfree::stack<int>>, std::ref(lockfreeStack));
    }
